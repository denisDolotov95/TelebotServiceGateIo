...
for log in (logging.getLogger(n) 
...
async def start():
    ...
    for conf in {ConfAlgorithm1, ConfAlgorithm4}:
        scheduler.add_job(
            auto_action,
            "interval",
            id=f"{conf.id}",
            minutes=conf.sched_interval,
            # seconds=5,
            kwargs={"type_algorithm": str(conf.id)},
        )
    ...
    info_3 = await ConfAlgorithm3.info()
    info_4 = await ConfAlgorithm4.info()
    info_5 = await ConfAlgorithm5.info()
